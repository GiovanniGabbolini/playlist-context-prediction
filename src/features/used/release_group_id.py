...
@cached_feature
@timing_feature
def release_group_id(source) -> "release_group_musicbrainz_id":
    ...
    Returns:
        dict -- release group id in musicbrainz.
    """
    if "album_name" in source["value"]:

        releases = musicbrainzngs.search_release_groups(
            source["value"]["album_name"],
            strict=True,
            artist=source["value"]["artist_name"],
        )["release-group-list"]
        filtered_releases = []
        for r in releases:
            try:
        ...
        if len(filtered_releases) > 0:
            ...
            logging.getLogger("root.features").warning(
                f"No release-group associate with album {source['value']['album_name']} by {source['value']['artist_name']}"
            )
            return None
...
def _release_groups_id(**kwargs):
    ...
    track_album_artists_id = (
        track_album_artists_id[["alid", "arid"]]
        .drop_duplicates()
        .groupby("alid")
        .head(1)
    )
    ...
    array_feature(
        release_group_musicbrainz_id, argument_values=argument_values, **kwargs
    )
