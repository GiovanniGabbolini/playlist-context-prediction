...
def find_segues(t1, t2, pre_filtering=pre, post_filtering=post, nodes_types_to_filter=nodes_types_to_filter_strict,  nodes_types_to_segue_not_equal=get_dict()):
    """Find all segues joining an entity1 to an entity2, repesented by trees t1 and t2.

    Notice: it assumes that t1 and t2 have as root the 'source' node.
    ...
    Args:
        ...
        nodes_types_to_filter (set, optional): t1 and t2 cannot be joyned by a node of these types.
    ...
    Returns:
        list: List of dictionaries. Each dictionary represent a segues, shaped as:
              {
                  'n1': ngx node, node from t1 that connects with 'n2' in t2
                  'n2': ngx node, node from t2 that connects with 'n1' in t1
                  'value': segue-type specific value
                  'compare_function': compare function stating that such a connection exists
              }

    If the order of trees is commuted, this method returns the same segues,
    i.e. segue representation that represent the same path, but reversed; e.g. segue type is equal but reversed.
    """
    # First, merges t1 and t2 in a unique graph.
    # Then, find segues as paths from the source node of t1 and the source node of t2.
    ...
    # Map back a tuple of nodes ids in g to a list of nodes in t1 (dictionary 0) and t2 (dictionary 1)
    # A series of identical nodes in g can be mapped to more nodes in one of the starting trees, we are in a multigraph scenario.
    map_back = {'t1': {}, 't2': {}}

    # Tells whether an edge in g was from t1 or t2 or
    # if it was induced, i.e. resulting from the application of a compare functio to nodes from t1 and t2
    edges = {'t1': set(), 't2': set(), 'induced': set()}
    ...
    # Here we merge the trees

    # Every node in t1 and t2 is represented by a string, which is the conversion of its fields to text (mergiable_id)
    # This automatically implements the equal compare function, as equal nodes will converge into the same node in g.
    for idx, addend in enumerate((t1, t2)):
        id_tree = f"source_{idx}"
        ...
        while stack:
            ...
            if child is None:
                ...
                if idx == 0:
                    ...
                    edges['t1'].add((children[0][-1], child_id))
                else:
                    g[child_id].add(children[0][-1])
                    edges['t2'].add((child_id, children[0][-1]))

                key = children[0]+(child_id,)
                if key in map_back[f't{idx+1}']:
                    map_back[f't{idx+1}'][key].append(child)
                else:
                    map_back[f't{idx+1}'][key] = [child]
    ...
    for type_1, type_2 in compareble_nodes_without_equal:
        ...
        nodes_type_1 = [t1.nodes()[node_id] for node_id in t1.nodes() if t1.nodes()[node_id]['type'] == type_1]
        nodes_type_2 = [t2.nodes()[node_id] for node_id in t2.nodes() if t2.nodes()[node_id]['type'] == type_2]
    ...
    # Find out which is the last node that belongs to t1 and which is the first that belongs to t2
    # middle_leg is len==2 tuple which has as values such information
    for j, path in enumerate(paths):
        for idx in range(2, len(path)):
            if tuple(path[:idx]) not in map_back['t1']:
                idx = idx-2
                middle_leg = (path[idx], path[idx+1])
                break

        if (tuple(path[idx:][::-1]) in map_back['t2']):
            # Compare function == equal
            for id_1, id_2 in itertools.product(map_back['t1'][tuple(path[0:idx+1])], map_back['t2'][tuple(path[idx:][::-1])]):

                segue = {'n1': t1._node[id_1],
                         'n2': t2._node[id_2],
                         'value': t1._node[id_1]['value'],
            ...
            # Compare function != equal
            for id_1, id_2 in itertools.product(map_back['t1'][tuple(path[0:idx+1])], map_back['t2'][tuple(path[idx+1:][::-1])]):

                candidated_segues = iter([{**{'n1': t1._node[id_1], 'n2': t2._node[id_2]}, **induced_edge_infos}
