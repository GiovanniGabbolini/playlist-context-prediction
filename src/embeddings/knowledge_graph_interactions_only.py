...
from pykg2vec.pykg2vec.pykg2vec.data.kgcontroller import KnowledgeGraph
from pykg2vec.pykg2vec.pykg2vec.common import Importer, KGEArgParser
from pykg2vec.pykg2vec.pykg2vec.utils.trainer import Trainer
...
with both just interactions data.
This method is opposed to the other method in the `knowledge_graph` file, which uses both interactions and metadata.
Infact, we would like to compare the impact on metadata so that we could justify their presence.

Things to run to get the embeddings:
`knowledge_graph_embedding` -> `tracks_embeddings`.
"""
...
def knowledge_graph_embedding():
    ...
    os.makedirs(
        f"{folder_path}/embeddings/knowledge_graph_interactions_only/graph_embedding",
        exist_ok=True,
    )
    ...
    # save triplets
    with open(
        f"{folder_path}/embeddings/knowledge_graph_interactions_only/graph_embedding/{dataset_name}-train.txt",
        "w",
    ) as f:
    ...
    with open(
        f"{folder_path}/embeddings/knowledge_graph_interactions_only/graph_embedding/{dataset_name}-valid.txt",
        "w",
    ) as f:
        f.write(s.split("\n")[-2])

    with open(
        f"{folder_path}/embeddings/knowledge_graph_interactions_only/graph_embedding/{dataset_name}-test.txt",
        "w",
    ) as f:
        f.write(s.split("\n")[-2])
    ...
    # TODO: run this following part in HPC machines.
    params = [
        "-mn",
        "transd",
        "-ds",
...
def tracks_embeddings():
    ...
    embeddings_path = f"{base_path}/embeddings/knowledge_graph_interactions_only"
