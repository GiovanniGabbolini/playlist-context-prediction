...
import torch
from src.datasets import EmbeddingsTrackLevel, load_embedding_dictionary
...
def run(params, training_data, test_data, return_metrics=True):
    """
    The shape of the net is the seq. audio model as in [1]. (CNN + SequenceCategoricalClassification).
    ...
    classification_model = SequenceCategoricalClassification().to(device)
    loss_fn = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(
        [*audio_model.parameters(), *classification_model.parameters()],
    ...
    while True:
        train_loop(
            train_dataloader, audio_model, classification_model, loss_fn, optimizer
        )
        predictions, ys = test_loop(
            test_dataloader, audio_model, classification_model, loss_fn
        )
...
def train_loop(dataloader, audio_model, classification_model, loss_fn, optimizer):
    device = "cuda" if torch.cuda.is_available() else "cpu"

    for n_batch, batch in enumerate(tqdm(dataloader)):

        audio = [sample[0].to(device) for sample in batch]
        label = [sample[1].to(device) for sample in batch]

        lengths = [sample.shape[0] for sample in audio]
        ...
        emb = torch.split(emb, lengths, dim=0)
...
def test_loop(dataloader, audio_model, classification_model, loss_fn):
    test_loss = 0
    device = "cuda" if torch.cuda.is_available() else "cpu"
