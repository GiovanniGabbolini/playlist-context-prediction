...
class AverageAudio(torch.nn.Module):
    def __init__(self):
        super(AverageAudio, self).__init__()

        self.net = torch.nn.Sequential(
            # layer 1
            torch.nn.Conv1d(128, 128, kernel_size=4, stride=1, padding=2),
            torch.nn.ReLU(),
            ...
            torch.nn.MaxPool1d(kernel_size=4),
            # layer 4
            torch.nn.Conv1d(128, 128, kernel_size=2, stride=1, padding=2),
            torch.nn.ReLU(),
            torch.nn.MaxPool1d(kernel_size=2),
            # layer 5
            torch.nn.Conv1d(128, 50, kernel_size=1, stride=1, padding=0),
            torch.nn.ReLU(),
            torch.nn.MaxPool1d(kernel_size=2),
        )
...
def test_loop(dataloader, audio_model, classification_model, loss_fn):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader):
            ...
            # record where every playlist ends before stacking
            split_idx = [sample.shape[0] for sample in X]
