...
from src.models.commons import CategoricalClassification, EmbeddingFusion, Sequence
...
def run(params, training_data, test_data, return_metrics=True):
    ...
    sequence_1, sequence_2 = Sequence().to(device), Sequence().to(device)
    loss_fn = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(
        [*audio_model.parameters(), *classification_model.parameters(), *fusion_model.parameters(), *sequence_1.parameters(), *sequence_2.parameters()],
    ...
    while True:
        train_loop(
            train_dataloader, audio_model, classification_model, fusion_model, sequence_1, sequence_2, loss_fn, optimizer
        )
        predictions, ys = test_loop(
            test_dataloader, audio_model, classification_model, fusion_model, sequence_1, sequence_2, loss_fn
        )
...
def train_loop(dataloader, audio_model, classification_model, fusion_model, sequence_1, sequence_2, loss_fn, optimizer):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    for n_batch, batch in enumerate(tqdm(dataloader)):
        ...
        # get playlists embeddings
        emb = torch.split(emb, split_idx, dim=0)

        # transformation 4 seq model
        X = pad_sequence(emb)
        X = pack_padded_sequence(X, split_idx, batch_first=False, enforce_sorted=False)

        # classify
        emb_audio = sequence_1(X)
...
def test_loop(dataloader, audio_model, classification_model, fusion_model, sequence_1, sequence_2, loss_fn):
    test_loss = 0
    device = "cuda" if torch.cuda.is_available() else "cpu"
